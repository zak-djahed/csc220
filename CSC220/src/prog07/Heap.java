...
public class Heap <E> extends AbstractQueue<E> {
    ...
    */
	...
	///
	while(!isRoot(index) && compare(index, parent(index)) < 0){
    ...
    */
	...
	///
	E last = theElements.remove(size() - 1);
	theElements.set(0, last);
	...
	while(isValid(left(index)) && compare(index, left) > 0 || isValid(right(index)) && compare(index, right) > 0){
		...
		/*
		if(right < theElements.size() && left < theElements.size()){
			...
			if(left < theElements.size() && compare(index, left) > 0){
				index = swap(index, left);
			}
			if(right < theElements.size() && compare(index, right) > 0){
				index = swap(index, right);
			}
		}
	index++;
