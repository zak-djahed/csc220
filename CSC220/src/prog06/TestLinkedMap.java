...
public class TestLinkedMap extends LinkedMap {
	...
	boolean testRemove(Entry [] input, Entry [] output, Entry entry){
		entry.previous = null;
		entry.next = null;
		set(input);
		try{
	...
	void testRemove(){
		...
		{
			Entry[] in = {x0, a1, b2, c3};
			Entry[] out = { a1, b2, c3};
			check(testRemove(in, out, x0));
		}
		{
			Entry[] in = {a1, x0, b2, c3};
			Entry[] out = {a1, b2, c3};
			check(testRemove(in, out, x0));
		}
		{
			Entry[] in = {a1, b2, x0, c3};
			Entry[] out = {a1, b2, c3};
			check(testRemove(in, out, x0));
		}
		{
			Entry[] in = {a1, b2, c3, x0};
			Entry[] out = {a1, b2, c3};
			check(testRemove(in, out, x0));
		}
	}
